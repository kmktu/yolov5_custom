...
def arg_parser():
    ...
    parser.add_argument('-n', '--num', type=int, required=True, help='NUMBER')
                ...
                try:
                    ...
                    if json_data.get("annotations"):
                        ...
                        for i in range(len(annotations)):
                            ...
                            processor_id = annotation["processor_id"]
                            ...
                            if not fnmatch.fnmatch(processor_id, "DL*"):
                                collect_id = False

                        # if not match for pattern to collect_id, modify collect_id
                        if collect_id == False:
                            sr = SystemRandom()
                            n = sr.choice(range(1000, 10000))
                            pin = format(n, '04')
                            random_id = "DL" + str(pin)

                            for i2 in range(len(annotations)):
                                annotation2 = annotations[i2]
                                annotation2["processor_id"] = random_id
                        collect_id = True
